...
> 2026-07-14 v0.2（ENG-1 调参结论）：① ante 的 scatter 权重系数由 ×2 改为 **×1.12**（触发概率 ∝ 权重⁴，×2 会使触发率暴涨 17 倍击穿 RTP；×1.12 ≈ 触发率 ×1.45）；② 连锁倍数阶梯超出末档 10 后每级 **+5**（15, 20, 25…），免费局按阶梯格位跨 spin 推进；③ 封顶 5000× 为**单 spin 语义**，免费旋转整段总和可超过（是否改为整段封顶待定）；④ 基础局 accumulatedMultiplierAfter 恒为 0。

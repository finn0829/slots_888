...
## 环节 9 · ADM-4a/4b 审计查询与盘面回放

- 实现：AuditPage（过滤栏玩家/日期/最小倍数 + 分页表 + 行内"回放"）；DetailView 渲染 6×5 盘面（CT-4 用色：朱砂中/萬、竹绿發/條、金牌渐变+角标倍数）、消除位置朱砂描边高亮、逐连锁步进、一致性徽章（绿=一致，朱砂=不一致）；`#/audit?playerId=N` 带参直达。
- 验证：tsc 过；Playwright 8 项全过（带参直达/30 张牌/一致徽章/步进/minWinX 过滤）；截图目检恰好抓到一局免费旋转——金牌 ×2 角标、累计倍数 ×5、回放一致，free 局回放口径实证有效。
- 决策：日期过滤 to 在前端 +1 天转成服务端的 `<` 语义，服务端保持半开区间不特判。
- 踩坑：无。
